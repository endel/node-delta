...
static void putInt(unsigned int v, char **pz){
  static const char zDigits[] = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz~";
...
int delta_create(
  ...
  */
  nHash = lenSrc/NHASH;
  collide = (int*) malloc( nHash*2*sizeof(int) );
...
void Create(const Nan::FunctionCallbackInfo<Value>& args) {
  const char* zOrig = node::Buffer::Data(args[0]);
  size_t lenOrig = node::Buffer::Length(args[0]);
...
void Init(Local<Object> exports) {
  ...
  exports->Set(Nan::New("apply").ToLocalChecked(),
      Nan::New<FunctionTemplate>(Apply)->GetFunction());
}

NODE_MODULE(delta, Init)
